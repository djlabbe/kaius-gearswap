...
do
	...
	function precastequip(spell)
		...
		if spell.type == 'WeaponSkill' then
			if sets.WS then
				built_set = set_combine(built_set, sets.WS) -- Merge the default WS Set
				local message = ''
				if spell.skill == "Marksmanship" or spell.skill == "Archery" then
					...
					if sets.WS[spell.english] then
						-- Set is defined
						built_set = set_combine(built_set, sets.WS[spell.english]) -- Merge the specific WS Set
	...
	function jobsetup(LockStylePallet,MacroBook,MacroSet)
		...
		send_command('bind f11 gs c AutoBuff')
		send_command('bind f12 gs c TreasureHunter')
		...
		windower.add_to_chat(8,'Stance - '..string.format('[%s]','F10'))
		windower.add_to_chat(8,'Auto Buff - '..string.format('[%s]','F11'))
		windower.add_to_chat(8,'TH Mode - '..string.format('[%s]','F12'))
		if UI_Name ~= '' then
	...
	function display_box_update()
		local width = 20
		local dialog = {}
		dialog[1] = {description = 'DPS', value = state.WeaponMode.value}
		dialog[2] = {description = 'Stance', value = state.OffenseMode.value}
		dialog[3] = {description = 'Auto Buff', value = state.AutoBuff.value}
		dialog[4] = {description = 'TH Mode', value = state.TreasureMode.value}
		if UI_Name ~= "" then
